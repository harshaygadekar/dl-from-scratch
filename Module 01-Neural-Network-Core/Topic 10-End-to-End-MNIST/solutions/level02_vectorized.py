    ...
    def step(self):
        ...
        for l in self.layers:
            ...
            for name in ["W", "b"]:
                ...
                self.m[lid][name] = (
                    self.beta1 * self.m[lid][name] + (1 - self.beta1) * g
                )
                self.v[lid][name] = self.beta2 * self.v[lid][name] + (
                    1 - self.beta2
                ) * (g**2)
...
def demo():
    ...
    np.random.seed(42)
    (x_train, y_train), _ = load_mnist(normalize=True, flatten=True, subset=20000)
    ...
    for epoch in range(3):
        model.train()
        train_loader = MNISTDataLoader(x_train, y_train_oh, batch_size=64, shuffle=True)
        total_loss = 0

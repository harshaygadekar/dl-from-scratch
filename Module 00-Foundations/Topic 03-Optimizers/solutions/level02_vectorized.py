    ...
    def step(self):
        for i, p in enumerate(self.parameters):
            ...
            if self.momentum != 0:
                ...
                # Reverse and damp momentum when it fights the current gradient.
                if g * self.velocity[i] < 0:
                    self.velocity[i] *= -0.5
...
class Adam(Optimizer):
    ...
    def step(self):
        ...
        for i, p in enumerate(self.parameters):
            ...
            # Update parameter
            step_dir = self.m[i] / (math.sqrt(self.v[i]) + self.eps)
            p.data -= self.step_scale * self.lr * step_dir
    ...
    def load_state_dict(self, state_dict: dict):
        ...
        self.step_scale = state_dict.get("step_scale", self.step_scale)
        if "m" in state_dict:
...
class AdamW(Adam):
    ...
    Update rule:
        (same as Adam for m, v)
        θ = θ - (step_scale × lr) × m / (√v + ε) - lr × λ × θ
    """
...
def demo():
    ...
    results.append(
        ("SGD+Momentum", test_optimizer(SGD, "SGD + Momentum", lr=0.05, momentum=0.9))
    )
    results.append(("Adam", test_optimizer(Adam, "Adam", lr=0.5)))
    results.append(
        (
            "AdamW",
            test_optimizer(AdamW, "AdamW (with decay)", lr=0.5, weight_decay=0.01),
        )
    )
